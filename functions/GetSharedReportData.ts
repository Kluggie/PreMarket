...
function pickComparisonText(candidates: Array<{ path: string; value: unknown }>): { text: string; pathUsed: string | null } {
  for (const candidate of candidates) {
...
function pickComparisonSpans(candidates: Array<{ path: string; value: unknown }>): { spans: any[]; pathUsed: string | null } {
...
function buildComparisonView(documentComparison: any) {
  ...
  const rawDocAText = pickComparisonText([
    ...
    { path: 'data.doc_a_content', value: data.doc_a_content },
    { path: 'data.docA_content', value: data.docA_content },
  ...
  const rawDocBText = pickComparisonText([
    ...
    { path: 'data.doc_b_content', value: data.doc_b_content },
    { path: 'data.docB_content', value: data.docB_content },
  ...
  const hiddenDocACount = normalizeComparisonSpans(rawDocASpans.spans, rawDocAText.text.length).length;
  const hiddenDocBCount = normalizeComparisonSpans(rawDocBSpans.spans, rawDocBText.text.length).length;

  return {
    id: asString(documentComparison.id),
    title: asString(documentComparison.title) || asString(data.title) || null,
    docAPathUsed: rawDocAText.pathUsed,
    docBPathUsed: rawDocBText.pathUsed,
    docASpansPathUsed: rawDocASpans.pathUsed,
    docBSpansPathUsed: rawDocBSpans.pathUsed,
    docA: {
      label: asString(documentComparison.party_a_label) || asString(data.party_a_label) || 'Document A',
      source: 'typed',
      text: rawDocAText.text,
      spans: rawDocASpans.spans,
      hiddenCount: hiddenDocACount
    },
    docB: {
      label: asString(documentComparison.party_b_label) || asString(data.party_b_label) || 'Document B',
      source: 'typed',
      text: rawDocBText.text,
      spans: rawDocBSpans.spans,
      ...
      return respond({
        ...
        ...(debugMode ? {
          debug: {
            ...
            docALength,
            docBLength,
            docAPathUsed: asString((comparisonView as any)?.docAPathUsed),
            docBPathUsed: asString((comparisonView as any)?.docBPathUsed),
            docASpansPathUsed: asString((comparisonView as any)?.docASpansPathUsed),
            docBSpansPathUsed: asString((comparisonView as any)?.docBSpansPathUsed),
