...
function buildWebParitySections(params: {
  ...
  if (sections.length === 0) {
    sections.push({
      ...
      paragraphs: [summary || 'No review content is available yet.'],
    });
  }
...
export default async function handler(req: any, res: any, comparisonIdParam?: string) {
  await withApiRoute(req, res, '/api/document-comparisons/[id]/download/pdf', async (context) => {
    ...
    const senderOnlyFooterNote = isPreSendReview
      ? 'Confidential -- Based only on current materials from one side'
      : 'Confidential -- Generated by PreMarket AI';
    ...
    if (pdfFormat === 'web-parity') {
      const reviewTitle = isPreSendReview
        ? PRE_SEND_REVIEW_TITLE
        : buildMediationReviewTitle(comparison.title, report.title, evaluationResult.title);
      const reviewSubtitle = isPreSendReview
        ? asText(comparison.title)
        : buildMediationReviewSubtitle(comparison.title, report.title, evaluationResult.title);
      ...
      const filenameBase = slugify(reviewTitle) || defaultFilenameBase;
      const filename =
      ...
      const pdfBuffer = await renderWebParityPdfBuffer({
        ...
        subtitle: isPreSendReview ? reviewSubtitle : '',
        ...
        footerNote: senderOnlyFooterNote,
    ...
    if (dynamicPresentationSections.length > 0) {
      ...
      reportSections.push({
        heading: isPreSendReview ? 'Findings' : 'Findings',
        level: 2,
    ...
    const pdfBuffer = await renderProfessionalPdfBuffer({
      ...
      comparisonId: comparison.id,
      decisionPanel: !isPreSendReview && isV2 ? decisionPanel : undefined,
      footerNote: senderOnlyFooterNote,
