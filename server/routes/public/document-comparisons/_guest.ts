...
export async function runGuestEvaluationModel(params: {
  ...
  const v2Result = await evaluateWithVertexV2({
    ...
    stage1SourceProvenance: {
      shared_source_types: ['guest_shared_material'],
      confidential_source_types: ['guest_private_material'],
      shared_response_count: params.docBText.trim() ? 1 : 0,
      confidential_response_count: params.docAText.trim() ? 1 : 0,
