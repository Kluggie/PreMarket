...
export function buildEvalPromptFromFactSheet(params: {
  ...
  return [
    ...
    '- "The parties appear broadly aligned on the pilot concept, but not yet on the level of detail needed for a fixed-price commitment."',
    '- "The remaining hesitation seems to sit around triage rules, acceptance definitions, and change-control scope."',
    '- "This looks more bridgeable than fundamentally misaligned."',
    '- "A short paid discovery step may be the cleanest way to reduce ambiguity before the pilot terms are fixed."',
    '- "The friction appears to be less about intent and more about how execution risk is currently being allocated."',
    ...
    '- Use confidential context to calibrate alignment: if both sides have more flexibility than the shared materials show, let your bridgeability judgment and tone reflect that — without revealing the evidence.',
    ...
    '2. Name the real hesitation: what is actually preventing commitment on each side? Classify each blocker by type: scope ambiguity, risk-allocation dispute, pricing/commercial gap, information gap, timeline conflict, or trust concern. Distinguish genuine blockers from ordinary later-stage detail that resolves during contracting.',
    '3. Judge bridgeability: for each real blocker from step 2, decide — bridgeable with reasonable effort, or fundamentally incompatible? A blocker is bridgeable if a concrete change in structure, scope, sequencing, or information exchange would resolve it. It is fundamental only if the parties want incompatible outcomes and no restructuring can reconcile them.',
    '4. Design the bridge: for each bridgeable blocker, name the specific deal mechanic that would unlock movement — phased engagement, paid discovery, conditional commitment, capped pilot, escrow/holdback, acceptance gate, scope carve-out, risk-sharing formula, or another structure that fits. Be concrete about the structure, not just "resolve this issue."',
    '5. What can be agreed now versus later: separate what must be resolved before commitment from what can safely be deferred to contracting or implementation. Only items that would change the decision belong in "now."',
    '6. Scope boundary & evidence: are deliverables concrete enough to price and sequence? Flag vague language: "ASAP", "scalable", "world-class", "top N" without definitions, "TBD".',
    '7. Feasibility / realism: are timeline, constraints, assumptions realistic and contractable?',
    '8. Acceptance & measurability: are KPIs / success criteria concrete for reliable sign-off?',
    '9. Risk allocation: who is carrying data, dependency, or change-order risk? How does that shape each side\u2019s hesitation?',
    '10. Negotiation dynamics: what leverage, tradeoffs, urgency, switching costs, or dependency signals shape each side\u2019s position?',
    '11. Confidential calibration: if confidential information reveals pricing flexibility, internal constraints, or approval dynamics, use that to calibrate how close the parties actually are — even though you cannot disclose it. Your output may note the gap appears narrower or wider than surface positions suggest, without revealing why.',
    '12. Decision-readiness: is this ready for a clean commitment, only for a conditional path, or not yet ready? Use source_coverage flags to guide your assessment.',
    hasPriorBilateralContext
      ? '13. Progress across rounds: because this is not the first bilateral review, track what changed since the prior round — what was resolved, what narrowed, what regressed, and whether the negotiation is converging, stalled, or diverging.'
      : '',
    '',
    ...
    '- "high" fit_level is RARE. Only when scope, acceptance, dependencies, and risk allocation are sufficiently bounded.',
