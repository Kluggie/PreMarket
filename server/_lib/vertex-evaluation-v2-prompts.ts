...
export function buildEvalPromptFromFactSheet(params: {
  ...
  return [
    ...
    'The Mediation Summary should be the dominant section. For most cases, 3-4 strong paragraphs in Mediation Summary plus 1-2 short supporting sections is the ideal structure.',
    '',
    '- Narrative mediation note (good default): 3-4 well-formed paragraphs in Mediation Summary. Open with a short orienting sentence on shared alignment, then a separate paragraph for the real sticking point, then bridgeability and path forward. Do not pack everything into one paragraph. Most cases should use this.',
    '- Decision-oriented note: more direct. Makes clear whether matter looks viable or blocked. Use when the issue is near deadlock or a meaningful turning point.',
    '- Negotiation-path note: emphasizes likely landing zone, what each side needs, proposes bridge or sequence. Use for scope, pricing, pilot, or staged-commitment situations.',
    '- Risk-and-bridge note: explains why each side may hesitate, then proposes how to reduce that hesitation. Use when a party is likely to hesitate due to vagueness or risk allocation.',
    '- Information-gap note: emphasizes uncertainty rather than conflict. Use when the gap is definitional or operational rather than adversarial.',
    '- Near-agreement note: affirms parties are close, identifies remaining points without alarm. Use when parties appear largely aligned.',
    '',
    'ADAPTIVE REPORT STRUCTURE:',
    '- The why[] array MUST always begin with "Mediation Summary: \u2026" — this is the backbone of the output.',
    '  Write 3-4 paragraphs with clear paragraph breaks between distinct ideas:',
    '  Paragraph 1: Short orientation — where the parties appear aligned and the shape of the deal.',
    '  Paragraph 2: The real sticking point — what is actually preventing commitment, in plain English.',
    '  Paragraph 3: Bridgeability — whether this looks fixable and the most realistic route forward.',
    '  Paragraph 4 (optional): Any additional nuance that does not fit above.',
    '  Do NOT compress all of these into one dense paragraph. Spread them out so the reader can absorb each idea separately.',
    ...
    '  Keep Decision Readiness to 1-2 sentences only. Do NOT repeat blockers or analysis already in the Mediation Summary.',
    '- Beyond those two required sections, add 1-3 additional sections ONLY if they contribute genuinely new insight not already in the Mediation Summary.',
    ...
    '- For most cases, 3-4 total why[] entries is sufficient. Only use 5 if the case has genuinely distinct dimensions to cover.',
    '- Prefer a strong Mediation Summary over many small sections. If content would be thin in its own section, fold it into the Mediation Summary instead.',
    ...
    '- RECOMMENDATION SECTION RULE: If you include a "Recommended Path" or "Suggested Next Step" section, it must ONLY describe the path forward. Do NOT restate earlier analysis, blockers, or issues. One short paragraph that says what to do next is enough. Think of it as an action line, not a second summary.',
    '',
    ...
    '- Additional headings: choose 1-3 adaptive headings only if they add genuinely new insight.',
    '- Total why[] array should contain 3-5 entries. Prefer fewer, stronger sections over many thin ones.',
    '',
    ...
    '- confidence_0_1 should reflect your genuine assessment of how certain you are in the fit_level recommendation. Every case is different — produce a value that is specific to THIS case, not a default.',
    '- There is no single correct confidence for conditional cases. A conditionally viable deal with one remaining issue deserves higher confidence than one with five material unknowns. Reflect this.',
    ...
    '- When material uncertainty remains, the narrative MUST explicitly read as conditional.',
