...
export function buildPreSendPromptFromFactSheet(params: {
  ...
  return [
    ...
    '- Treat readiness with commercial nuance: a draft may be suitable for early vendor discussion while still being unready for a reliable fixed-price, pilot, or final commercial commitment.',
    ...
    '- send_readiness_summary must be a short evidence-based paragraph that explains what the draft is good enough for now, what it is not yet strong enough for, and the 1-3 main reasons.',
    '- Prefer nuanced wording such as "suitable for early vendor discussion, but not yet ready for a reliable fixed-price commitment" when that better matches the evidence.',
