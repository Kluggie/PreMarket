      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-white border border-slate-200 p-1 mb-6 flex-wrap">
            ...
            {isPartyB && (
              <TabsTrigger value="verify" className="data-[state=active]:bg-slate-900 data-[state=active]:text-white">
          ...
          {/* Verify Tab - Recipient Only */}
          {isPartyB && (
            <TabsContent value="verify">
              <Card className="border-0 shadow-sm border-l-4 border-l-blue-500 mb-6">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <Shield className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium text-blue-900">Recipient Verification</p>
                      <p className="text-sm text-blue-700 mt-1">
                        Review each field provided by the proposer. Mark as Confirmed (accurate), Disputed (incorrect), or Unknown (cannot verify).
                        Your verification helps build trust and improves evaluation accuracy.
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <div className="space-y-4">
                {responses.map(response => {
                  const verification = verifications.find(v => v.question_id === response.question_id);
                  const [verifyStatus, setVerifyStatus] = React.useState(verification?.status || null);
                  const [verifyNotes, setVerifyNotes] = React.useState(verification?.notes || '');

                  const handleVerify = async (status) => {
                    setVerifyStatus(status);
                    
                    if (verification) {
                      await base44.entities.VerificationItem.update(verification.id, {
                        status,
                        notes: verifyNotes
                      });
                    } else {
                      await base44.entities.VerificationItem.create({
                        proposal_id: proposalId,
                    ...
                    queryClient.invalidateQueries(['verifications', proposalId]);
                  };
                    ...
                    <Card key={response.id} className="border-0 shadow-sm">
                      <CardContent className="p-4">
                        ...
                        <div className="flex items-center gap-2 mt-4">
                          ...
                          <Button
                            size="sm"
                            variant={verifyStatus === 'confirmed' ? 'default' : 'outline'}
                            className={verifyStatus === 'confirmed' ? 'bg-green-600 hover:bg-green-700' : ''}
                          ...
                          <Button
                            size="sm"
                            variant={verifyStatus === 'disputed' ? 'default' : 'outline'}
                            className={verifyStatus === 'disputed' ? 'bg-red-600 hover:bg-red-700' : ''}
                          ...
                          <Button
                            size="sm"
                            variant={verifyStatus === 'pending' ? 'default' : 'outline'}
                        ...
                        {verifyStatus && (
                          <div className="mt-3">
                            <Textarea
                              ...
                              onBlur={() => {
                                if (verification) {
                                  base44.entities.VerificationItem.update(verification.id, {
                                    notes: verifyNotes
                                  });
                                  queryClient.invalidateQueries(['verifications', proposalId]);
                                }
