...
export function getAppendixOpenQuestions(report) {
  ...
  return missingItems.filter((item) => {
    ...
    // Keyword overlap check — if >70% of the question's key words already
    // appear in the rendered narrative, the question is redundant. Use a high
    // threshold so domain vocabulary overlap (scope, pricing, acceptance) does
    // not accidentally filter genuinely new decision-blocking questions.
    ...
    if (itemWords.length >= 3) {
      ...
      if (matchCount / itemWords.length >= 0.7) return false;
    }
    return true;
  });
}
